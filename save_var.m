...
for id_Inj = 1:num_Inj                                                                          %id of injector
    for id_Cyc = 1:num_Cyc                                                                      %id of injection cycle
        ...
        sig_a = eval(sprintf('ti%ia_l_msg__%i_',id_Cyc,id_Inj-1));                              % signal a and b of the same Inj, Cyc
        sig_b = eval(sprintf('HDEV_Zylinder_%i_Out6_%i_',id_Inj,id_Cyc-1));
        sig_b = sig_b*1000;                                                                     % switch s to ms
        
        bit_keep = [];                                                                          % initialization deletion array
        
        index_nonzero = find(sig_a~=0);                                                         % delete indices for a, where sig_a == 0
        sig_a_nonzero = sig_a(index_nonzero);                                                   % all the data left from sig_a are valid
        time_a_nonzero = t_ECU(index_nonzero);
        
        for k = 1:length(sig_b)                                                                 % delete indices for b, where sig_a == 0
            t_b_k = t_HiL(k,1);                                                                 % time stamp of sig_b to be checked
            flag = find(t_ECU == t_b_k);                                                        % if there is an exact match
            if isempty(flag)                                                                    % t_b_k is not in time_0                                                                  
                [close_a, id_close_a] = closest(t_b_k, t_ECU);                                  % find the closest value in t_ECU to t_b_k
                if t_ECU(id_close_a) > t_b_k
                    bit_position = [id_close_a-1 id_close_a];                                   % find the closest time stamps in t_ECU to t_b_k
                else
                    bit_position = [id_close_a id_close_a+1];
                end

                if ismember(bit_position, index_nonzero)                                        % only keep t_b_k, when both closest time stamps are in index_nonzero
                    bit_keep(end+1) = k;
                else
                    % delete k
                end
        ...
        a_and_b{id_Inj,id_Cyc} = [sig_a_valid sig_b_valid time_valid];                          % read a, b and time into struct
